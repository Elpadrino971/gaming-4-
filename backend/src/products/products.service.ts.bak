...
export class ProductsService {
  ...
  async createProduct(dto: CreateProductDto) {
    return this.prisma.product.create({
      data: {
        ...
        images: dto.images || [],
        priceInCredits: new Prisma.Decimal(dto.priceInCredits),
        priceInEur: dto.priceInEur ? new Prisma.Decimal(dto.priceInEur) : null,
        stock: dto.stock || 0,
  ...
  async updateProduct(id: string, dto: UpdateProductDto) {
    ...
    if (dto.priceInCredits !== undefined) {
      updateData.priceInCredits = new Prisma.Decimal(dto.priceInCredits);
    }
    if (dto.priceInEur !== undefined) {
      updateData.priceInEur = dto.priceInEur ? new Prisma.Decimal(dto.priceInEur) : null;
    }
  ...
  async findProductForPrizePool(prizePoolInCredits: number) {
    ...
    if (products.length === 0) {
      ...
      const closestProduct = await this.prisma.product.findFirst({
        where: {
          status: 'ACTIVE',
          priceInCredits: { lte: new Prisma.Decimal(prizePoolInCredits) },
          OR: [
            { unlimited: true },
            { stock: { gt: 0 } },
          ],
